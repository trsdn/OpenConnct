...
plugin="com.trsdn.openconnct.sdPlugin"
...
# OpenDeck runs plugins with whatever `node` is on PATH. (Stream Deck brings its
# own, so this only matters for OpenDeck.) A deck app started from the Dock or at
# login often has a shorter PATH than a terminal does, so a node that works here
# can still be invisible there: say where this one lives.
