...
DRIVER_BUNDLE="$BUILD_DIR/OpenConnct.driver"
DRIVER_BIN="$DRIVER_BUNDLE/Contents/MacOS/OpenConnct"
...
clang -x c -std=c11 -arch arm64 -arch x86_64 -mmacosx-version-min=13.0 \
  ...
  -o "$DRIVER_BIN" "$ROOT_DIR/App/OpenConnctDriver/OpenConnctDriver.c"
cp "$ROOT_DIR/App/OpenConnctDriver/Info.plist" "$DRIVER_BUNDLE/Contents/Info.plist"
