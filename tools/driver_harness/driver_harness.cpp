...
static bool check_device(Driver& d, AudioObjectID deviceID, const char* name, const char* uid, bool hidden, bool canDefault,
    ...
    if (man.status == noErr && man.size == sizeof(CFStringRef)) { CFStringRef s = *(CFStringRef*)man.bytes.data(); CHECK(same_cfstring(s, "OpenConnct"), "Device %u manufacturer is OpenConnct", deviceID); if (s) CFRelease(s); }
...
int main(int argc, char** argv) {
    ...
    std::string binary = std::string(argv[1]) + "/Contents/MacOS/OpenConnct";
    ...
    Factory create = (Factory)dlsym(handle, "OpenConnct_Create");
    REQUIRE_MAIN(create != nullptr, "dlsym OpenConnct_Create");
    ...
    if (manufacturer.status == noErr && manufacturer.size == sizeof(CFStringRef)) {
        ...
        CHECK(same_cfstring(s, "OpenConnct"), "Plug-in manufacturer is OpenConnct");
        if (s) CFRelease(s);
    }
    ...
    CFStringRef micUID = CFSTR("OpenConnctMic_UID");
    CFStringRef sinkUID = CFSTR("OpenConnctSink_UID");
    CFStringRef unknownUID = CFSTR("DefinitelyNotOpenConnct_UID");
    ...
    check_device(d, kObjectID_Device_Mic, "OpenConnct Mic", "OpenConnctMic_UID", false, true, 1, kObjectID_Stream_Mic, 0, 0);
    check_device(d, kObjectID_Device_Sink, "OpenConnct Sink", "OpenConnctSink_UID", true, false, 0, 0, 1, kObjectID_Stream_Sink);
