...
**Paths.** Commands in Sections 1–6 assume the repository is at `/Volumes/big/dev/OpenConnct`. If
...
**You will need:** two USB microphones, a USB cable for each, and your
...
The script copies the bundle to `/Library/Audio/Plug-Ins/HAL/OpenConnct.driver`, sets root
...
```bash
system_profiler SPAudioDataType | grep -A6 "OpenConnct"
```
...
```
        OpenConnct Mic:
          ...
          Input Channels: 2
          Manufacturer: OpenConnct
          Current SampleRate: 48000
          Transport: Virtual
```

You should see **`OpenConnct Mic`**. You should **not** see `OpenConnct Sink` — it is deliberately
hidden so that it cannot be selected as an output device and create a feedback loop. Its absence is
correct behaviour.

**b. From the GUI:** open **System Settings → Sound → Input**. `OpenConnct Mic` should be listed.
...
1. Confirm the bundle is actually on disk and owned by root:
   ...
   ```bash
   ls -l@ /Library/Audio/Plug-Ins/HAL/OpenConnct.driver/Contents/MacOS/OpenConnct
   ```
...
3. Read the actual rejection reason. Open **Console.app**, put `coreaudiod` in the search box, then
   ...
   anything naming OpenConnct, a code-signature failure, or a plug-in load error.
...
```bash
open /Volumes/big/dev/OpenConnct/dist/OpenConnct.app
```
...
>
> ```bash
> log show --last 2m --predicate 'process == "OpenConnct"' --style compact | grep -c AUHAL
> ```
>
...
>
> ```bash
> tccutil reset Microphone audio.openconnct.app
> ```
>
...
Audio Recording**, click the chevron next to the record button, and choose **OpenConnct Mic**. Its
...
| Status line red / not Ready | The engine failed to start; the sink device is probably missing. Check `OpenConnct Sink` exists internally by re-running the install |
...
Plug in both. Two microphones on two independent clocks is the case this project exists to get right.
...
t=60s   under=0 over=0 dropped=0
        [USB Microphone A  fill 1535/1535.9/1537  ppm -3.9/-0.5/+3.6]
        [USB Microphone B  fill 1535/1535.9/1537  ppm -3.9/-0.5/+3.6]
```
...
OPENCONNCT_SOAK_LOG=60 /Applications/OpenConnct.app/Contents/MacOS/OpenConnct 2>&1 | grep -a SOAK
```
...
OPENCONNCT_METER_HZ=0 /Applications/OpenConnct.app/Contents/MacOS/OpenConnct
```
...
`OPENCONNCT_METER_HZ=0` it drops to under 1% either way. If you see substantially more than that,
...
```bash
sudo rm -rf "/Library/Audio/Plug-Ins/HAL/OpenConnct.driver"
...
```bash
system_profiler SPAudioDataType | grep -c "OpenConnct"   # expect 0
```
...
1. **Remove the driver** with the two commands at the top of this section
   (`sudo rm -rf …/OpenConnct.driver` then `sudo killall -9 coreaudiod`). This alone fixes almost
...
6. **If audio is still broken after a reboot with the driver removed**, then the cause is not
   OpenConnct — the bundle is gone and nothing of ours is loaded. Check System Settings → Sound for
   a sensible output device, and look for a stuck app holding the device exclusively.

### Confirming you are fully clean

```bash
ls /Library/Audio/Plug-Ins/HAL/                            # no OpenConnct.driver
system_profiler SPAudioDataType | grep -c "OpenConnct"    # 0
```
...
`~/Library/Application Support/OpenConnct/`, which is safe to delete.
...
1. Whether `OpenConnct Mic` appeared, and what `system_profiler` printed for it.
