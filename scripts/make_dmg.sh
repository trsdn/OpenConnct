...
DMG_PATH="${DMG_PATH:-$DIST_DIR/OpenConnect-macos.dmg}"
STAGING_DIR="$DIST_DIR/dmg-staging"

if [[ ! -d "$APP_PATH" ]]; then echo "App bundle not found at $APP_PATH" >&2; exit 1; fi
if [[ ! -f "$PKG_PATH" ]]; then echo "Driver installer pkg not found at $PKG_PATH. Run scripts/make_pkg.sh first." >&2; exit 1; fi
