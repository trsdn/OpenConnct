//  OpenConnctDriver.c
//
//  CoreAudio AudioServerPlugIn for OpenConnct.
//
...
//  lives in OpenConnct.app.
//
//  Two devices are published, backed by the same ring buffer:
//
//    "OpenConnct Mic"   visible, input-only   -> what Teams/Zoom/OBS select
//    "OpenConnct Sink"  hidden,  output-only  -> what OpenConnct.app renders into
//
...
#define kPlugIn_BundleID            "audio.openconnct.driver"
#define kManufacturer_Name          CFSTR("OpenConnct")

#define kMicDevice_Name             CFSTR("OpenConnct Mic")
#define kMicDevice_UID              CFSTR("OpenConnctMic_UID")
#define kMicDevice_ModelUID         CFSTR("OpenConnctMic_ModelUID")

#define kSinkDevice_Name            CFSTR("OpenConnct Sink")
#define kSinkDevice_UID             CFSTR("OpenConnctSink_UID")
#define kSinkDevice_ModelUID        CFSTR("OpenConnctSink_ModelUID")
...
#define kOpenConnctCustomProperty_Version  'ocvs'
...
static void OpenConnct_InitTimebase(void)
{
...
static Boolean OpenConnct_IsDeviceID(AudioObjectID inObjectID)
{
    return (inObjectID == kObjectID_Device_Mic) || (inObjectID == kObjectID_Device_Sink);
}

static Boolean OpenConnct_IsStreamID(AudioObjectID inObjectID)
{
    return (inObjectID == kObjectID_Stream_Mic) || (inObjectID == kObjectID_Stream_Sink);
}

static Boolean OpenConnct_IsInputObject(AudioObjectID inObjectID)
{
    return (inObjectID == kObjectID_Device_Mic) || (inObjectID == kObjectID_Stream_Mic);
}

static AudioObjectID OpenConnct_StreamForDevice(AudioObjectID inDeviceID)
{
    return (inDeviceID == kObjectID_Device_Mic) ? kObjectID_Stream_Mic : kObjectID_Stream_Sink;
}

static void OpenConnct_FillStreamFormat(AudioStreamBasicDescription* outFormat)
{
...
static HRESULT  OpenConnct_QueryInterface(void* inDriver, REFIID inUUID, LPVOID* outInterface);
static ULONG    OpenConnct_AddRef(void* inDriver);
static ULONG    OpenConnct_Release(void* inDriver);
static OSStatus OpenConnct_Initialize(AudioServerPlugInDriverRef inDriver, AudioServerPlugInHostRef inHost);
static OSStatus OpenConnct_CreateDevice(AudioServerPlugInDriverRef inDriver, CFDictionaryRef inDescription, const AudioServerPlugInClientInfo* inClientInfo, AudioObjectID* outDeviceObjectID);
static OSStatus OpenConnct_DestroyDevice(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID);
static OSStatus OpenConnct_AddDeviceClient(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, const AudioServerPlugInClientInfo* inClientInfo);
static OSStatus OpenConnct_RemoveDeviceClient(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, const AudioServerPlugInClientInfo* inClientInfo);
static OSStatus OpenConnct_PerformDeviceConfigurationChange(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, UInt64 inChangeAction, void* inChangeInfo);
static OSStatus OpenConnct_AbortDeviceConfigurationChange(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, UInt64 inChangeAction, void* inChangeInfo);
static Boolean  OpenConnct_HasProperty(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress);
static OSStatus OpenConnct_IsPropertySettable(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, Boolean* outIsSettable);
static OSStatus OpenConnct_GetPropertyDataSize(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32* outDataSize);
static OSStatus OpenConnct_GetPropertyData(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32 inDataSize, UInt32* outDataSize, void* outData);
static OSStatus OpenConnct_SetPropertyData(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32 inDataSize, const void* inData);
static OSStatus OpenConnct_StartIO(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, UInt32 inClientID);
static OSStatus OpenConnct_StopIO(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, UInt32 inClientID);
static OSStatus OpenConnct_GetZeroTimeStamp(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, UInt32 inClientID, Float64* outSampleTime, UInt64* outHostTime, UInt64* outSeed);
static OSStatus OpenConnct_WillDoIOOperation(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, UInt32 inClientID, UInt32 inOperationID, Boolean* outWillDo, Boolean* outWillDoInPlace);
static OSStatus OpenConnct_BeginIOOperation(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, UInt32 inClientID, UInt32 inOperationID, UInt32 inIOBufferFrameSize, const AudioServerPlugInIOCycleInfo* inIOCycleInfo);
static OSStatus OpenConnct_DoIOOperation(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, AudioObjectID inStreamObjectID, UInt32 inClientID, UInt32 inOperationID, UInt32 inIOBufferFrameSize, const AudioServerPlugInIOCycleInfo* inIOCycleInfo, void* ioMainBuffer, void* ioSecondaryBuffer);
static OSStatus OpenConnct_EndIOOperation(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, UInt32 inClientID, UInt32 inOperationID, UInt32 inIOBufferFrameSize, const AudioServerPlugInIOCycleInfo* inIOCycleInfo);

#pragma mark - Interface

static AudioServerPlugInDriverInterface gInterface = {
    NULL,
    OpenConnct_QueryInterface,
    OpenConnct_AddRef,
    OpenConnct_Release,
    OpenConnct_Initialize,
    OpenConnct_CreateDevice,
    OpenConnct_DestroyDevice,
    OpenConnct_AddDeviceClient,
    OpenConnct_RemoveDeviceClient,
    OpenConnct_PerformDeviceConfigurationChange,
    OpenConnct_AbortDeviceConfigurationChange,
    OpenConnct_HasProperty,
    OpenConnct_IsPropertySettable,
    OpenConnct_GetPropertyDataSize,
    OpenConnct_GetPropertyData,
    OpenConnct_SetPropertyData,
    OpenConnct_StartIO,
    OpenConnct_StopIO,
    OpenConnct_GetZeroTimeStamp,
    OpenConnct_WillDoIOOperation,
    OpenConnct_BeginIOOperation,
    OpenConnct_DoIOOperation,
    OpenConnct_EndIOOperation
};

static AudioServerPlugInDriverInterface*    gInterfacePtr = &gInterface;
static AudioServerPlugInDriverRef           gInstance = &gInterfacePtr;

void*   OpenConnct_Create(CFAllocatorRef inAllocator, CFUUIDRef inRequestedTypeUUID);
void*   OpenConnct_Create(CFAllocatorRef inAllocator, CFUUIDRef inRequestedTypeUUID)
{
...
static Boolean OpenConnct_HasPlugInProperty(const AudioObjectPropertyAddress* inAddress)
{
    switch (inAddress->mSelector) {
        ...
        case kOpenConnctCustomProperty_Version:
            return true;
        default:
            return false;
    }
...
static OSStatus OpenConnct_GetPlugInPropertyDataSize(const AudioObjectPropertyAddress* inAddress, UInt32* outDataSize)
{
    switch (inAddress->mSelector) {
        case kAudioObjectPropertyBaseClass:
        case kAudioObjectPropertyClass:
...
static OSStatus OpenConnct_GetPlugInPropertyData(const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32 inDataSize, UInt32* outDataSize, void* outData)
{
    switch (inAddress->mSelector) {
        case kAudioObjectPropertyBaseClass:
        ...
        case kAudioObjectPropertyCustomPropertyInfoList: {
            ...
            info->mSelector = kOpenConnctCustomProperty_Version;
...
static Boolean OpenConnct_HasDeviceProperty(const AudioObjectPropertyAddress* inAddress)
{
    switch (inAddress->mSelector) {
        case kAudioObjectPropertyBaseClass:
        case kAudioObjectPropertyClass:
...
static UInt32 OpenConnct_StreamCountForScope(AudioObjectID inDeviceID, AudioObjectPropertyScope inScope)
{
...
static OSStatus OpenConnct_GetDevicePropertyDataSize(AudioObjectID inObjectID, const AudioObjectPropertyAddress* inAddress, UInt32* outDataSize)
{
    switch (inAddress->mSelector) {
        case kAudioObjectPropertyBaseClass:
        case kAudioObjectPropertyClass:
        ...
        case kAudioObjectPropertyOwnedObjects:
        case kAudioDevicePropertyStreams:
            *outDataSize = OpenConnct_StreamCountForScope(inObjectID, inAddress->mScope) * sizeof(AudioObjectID);
            return noErr;
        case kAudioObjectPropertyControlList:
            *outDataSize = 0;
            return noErr;
...
static OSStatus OpenConnct_GetDevicePropertyData(AudioObjectID inObjectID, const AudioObjectPropertyAddress* inAddress, UInt32 inDataSize, UInt32* outDataSize, void* outData)
{
    ...
    switch (inAddress->mSelector) {
        ...
        case kAudioObjectPropertyOwnedObjects:
        case kAudioDevicePropertyStreams: {
            UInt32 count = OpenConnct_StreamCountForScope(inObjectID, inAddress->mScope);
            UInt32 capacity = inDataSize / sizeof(AudioObjectID);
            if (count > capacity) { count = capacity; }
            if (count > 0) { ((AudioObjectID*)outData)[0] = OpenConnct_StreamForDevice(inObjectID); }
...
static Boolean OpenConnct_HasStreamProperty(const AudioObjectPropertyAddress* inAddress)
{
    switch (inAddress->mSelector) {
        case kAudioObjectPropertyBaseClass:
        case kAudioObjectPropertyClass:
...
static OSStatus OpenConnct_GetStreamPropertyDataSize(const AudioObjectPropertyAddress* inAddress, UInt32* outDataSize)
{
    switch (inAddress->mSelector) {
        case kAudioObjectPropertyBaseClass:
        case kAudioObjectPropertyClass:
...
static OSStatus OpenConnct_GetStreamPropertyData(AudioObjectID inObjectID, const AudioObjectPropertyAddress* inAddress, UInt32 inDataSize, UInt32* outDataSize, void* outData)
{
    Boolean isInput = OpenConnct_IsInputObject(inObjectID);
    ...
    switch (inAddress->mSelector) {
        ...
        case kAudioStreamPropertyPhysicalFormat:
            ...
            OpenConnct_FillStreamFormat((AudioStreamBasicDescription*)outData);
            *outDataSize = sizeof(AudioStreamBasicDescription);
            return noErr;
        ...
        case kAudioStreamPropertyAvailablePhysicalFormats: {
            ...
            OpenConnct_FillStreamFormat(&desc->mFormat);
...
static Boolean OpenConnct_HasProperty(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress)
{
    ...
    if (inObjectID == kObjectID_PlugIn)         { return OpenConnct_HasPlugInProperty(inAddress); }
    if (OpenConnct_IsDeviceID(inObjectID))     { return OpenConnct_HasDeviceProperty(inAddress); }
    if (OpenConnct_IsStreamID(inObjectID))     { return OpenConnct_HasStreamProperty(inAddress); }
    return false;
}
...
static OSStatus OpenConnct_IsPropertySettable(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, Boolean* outIsSettable)
{
    ...
    if (!OpenConnct_HasProperty(inDriver, inObjectID, inClientProcessID, inAddress)) {
        return kAudioHardwareUnknownPropertyError;
    }
...
static OSStatus OpenConnct_GetPropertyDataSize(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32* outDataSize)
{
    ...
    if (inObjectID == kObjectID_PlugIn)     { return OpenConnct_GetPlugInPropertyDataSize(inAddress, outDataSize); }
    if (OpenConnct_IsDeviceID(inObjectID)) { return OpenConnct_GetDevicePropertyDataSize(inObjectID, inAddress, outDataSize); }
    if (OpenConnct_IsStreamID(inObjectID)) { return OpenConnct_GetStreamPropertyDataSize(inAddress, outDataSize); }
...
static OSStatus OpenConnct_GetPropertyData(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32 inDataSize, UInt32* outDataSize, void* outData)
{
    ...
    if (inObjectID == kObjectID_PlugIn) {
        return OpenConnct_GetPlugInPropertyData(inAddress, inQualifierDataSize, inQualifierData, inDataSize, outDataSize, outData);
    }
    if (OpenConnct_IsDeviceID(inObjectID)) {
        return OpenConnct_GetDevicePropertyData(inObjectID, inAddress, inDataSize, outDataSize, outData);
    }
    if (OpenConnct_IsStreamID(inObjectID)) {
        return OpenConnct_GetStreamPropertyData(inObjectID, inAddress, inDataSize, outDataSize, outData);
    }
    return kAudioHardwareBadObjectError;
}
