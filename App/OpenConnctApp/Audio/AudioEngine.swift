...
/// output unit rendering into the hidden "OpenConnct Sink" device published by
/// our HAL plug-in.
///
...
final class AudioEngine {
    ...
    func requestPermissionAndStart(_ completion: @escaping (Bool) -> Void) {
        ...
        NSLog("OpenConnct: microphone authorizationStatus = %d (%@), bundle = %@",
        ...
        case .notDetermined:
            AVCaptureDevice.requestAccess(for: .audio) { granted in
                NSLog("OpenConnct: requestAccess returned granted = %@", granted ? "true" : "false")
    ...
    private func rebind(devices: [AudioInputDevice]) {
        ...
        NSLog("OpenConnct: %d input(s) available, %d bound: %@",
              availableDevices.count, usable.count,
              usable.map(\.name).joined(separator: ", "))
        ensureOutputRunning()
        NSLog("OpenConnct: sinkAvailable = %@, outputUnit = %@, inputUnits = %d",
